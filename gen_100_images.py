...
def train(rank, gpu, args):
    ...
    samples_folder = save_dir + '/samples'
    ...
    #     break
    x_t_1 = torch.randn(100, 3, args.image_size, args.image_size).to(device)
    fake_sample = sample_from_model(pos_coeff, netG, args.num_timesteps, x_t_1, T, args)
    fake_sample = to_range_0_1(fake_sample)
    for j, x in enumerate(fake_sample):
        torchvision.utils.save_image(x, f'{samples_folder}/{j}.jpg')
    # torchvision.utils.save_image(fake_sample, os.path.join(save_dir, 'sample_discrete_epoch_{}.png'.format(args.epoch)), normalize=True)
