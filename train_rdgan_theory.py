...
def train(rank, gpu, args):
    ...
    # 'clipart', 'quickdraw', 'sketch'
    if args.dataset in ['cifar10', 'mnist', 'stackmnist', 'stl10', 'celeba_256', 'clipart_64', 'quickdraw_64', 'sketch_64', 'isolation_forest_celeba_5p_fashion', 'mnist_1c', 'fashion_mnist_1c', 'mnist_cifar10', 'mnist12']:
        ...
        netD = Discriminator_64(nc = 2*args.num_channels, ngf = args.ngf,
                               t_emb_dim = args.t_emb_dim,
                               act=nn.LeakyReLU(0.2)).to(device)
    else:
    ...
    if exp == 'none':
        exp_path = parent_dir
        # print(exp_path)
        # print(os.path.join(parent_dir,exp))
    else:
    ...
    for epoch in range(init_epoch, args.num_epoch+1):
        ...
        for iteration, (x, y) in enumerate(tqdm(data_loader)):
            ...
            output = netD(x_0_predict, t, x_tp1.detach())
                
            errD_fake = phi_star1(output - args.tau * torch.sum(((x_0_predict-real_data.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1))
            # errD_fake = phi_star1(output - args.tau * torch.sum(((x_pos_sample-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1))
            # errD_fake = phi_star1(output - args.tau * torch.sum(((x_0_predict-noise).view(noise.size(0), -1))**2, dim=1))
            ...
            errG = args.tau * torch.sum(((x_0_predict-real_data.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1) - output
            # errG = args.tau * torch.sum(((x_pos_sample-x_tp1.detach()).view(x_tp1.detach().size(0), -1))**2, dim=1) - output
            # errG = args.tau * torch.sum(((x_0_predict-noise).view(noise.size(0), -1))**2, dim=1) - output
            errG = errG.mean()
