...
def main(args):
    ...
    for epoch in range(0, args.num_epoch+1): 
        ...
        for iteration, x in enumerate(tar_dataloader):
            ...
            t1 = int(args.batch_size / args.num_timesteps * args.t)
            t2 = int(args.batch_size - t1)

            
            #sample t
            t = torch.randint(0, args.t, (real_data[:t1].size(0),), device=device)
            
            x_t, x_tp1, _ = q_sample_pairs(coeff, real_data[:t1], t)
            ...
            # train with fake
            latent_z = torch.randn(t1, args.nz, device=device)
            ...
            netG.zero_grad()
            t = torch.randint(0, args.num_timesteps, (real_data[:t1].size(0),), device=device)
            x_t, x_tp1, _ = q_sample_pairs(coeff, real_data[:t1], t)
            latent_z = torch.randn(t1, args.nz, device=device)
            x_0_predict = netG(x_tp1.detach(), t, latent_z)
            x_pos_sample, _ = sample_posterior(pos_coeff, x_0_predict, x_tp1, t)
            ...
            #sample t
            t = torch.randint(args.t, args.num_timesteps, (real_data[t1:args.batch_size].size(0),), device=device)
            
            x_t, x_tp1, _ = q_sample_pairs(coeff, real_data[t1:args.batch_size], t)
            x_t.requires_grad = True
            ...
            # train with fake
            latent_z = torch.randn(t2, args.nz, device=device)
            ...
            for p in netD.parameters():
                p.requires_grad = False
            netG.zero_grad()
            t = torch.randint(0, args.num_timesteps, (real_data[t1:args.batch_size].size(0),), device=device)
            x_t, x_tp1, _ = q_sample_pairs(coeff, real_data[t1:args.batch_size], t)
            latent_z = torch.randn(t2, args.nz, device=device)
            x_0_predict = netG(x_tp1.detach(), t, latent_z)
            x_pos_sample, _ = sample_posterior(pos_coeff, x_0_predict, x_tp1, t)
