...
#%%
def sample_and_test(args, epoch_id, num_step):
    ...
    result_folder = result_dir + f'/{args.version}/fid_{num_step}'
    ...
    elif args.dataset == 'celeba_256' or args.dataset == 'isolation_forest_celeba_5p_fashion':
    ...
    json_filename = f'{result_folder}/epoch_{epoch_id}_output.json'
    print(json_filename)
    ...
    if args.compute_fid:
        for i in range(iters_needed):
            with torch.no_grad():
                ...
                fake_sample = sample_from_model(pos_coeff, netG, num_step, x_t_1,T,  args)
                
                fake_sample = to_range_0_1(fake_sample)
                for j, x in enumerate(fake_sample):
                    index = i * args.batch_size + j 
                    torchvision.utils.save_image(x, f'{samples_folder}/{index}.jpg')
                    # torchvision.utils.save_image(x, f'./generated_samples/{args.dataset}/{index}.jpg')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--index', type=str, default='1',
                        help='also version, but for the saved results')
    ...
    for epoch_id in range(args.epoch_start, args.epoch_end + 1, args.epoch_jump):
        for num_step in range(1, args.num_timesteps):
            sample_and_test(args=args, epoch_id=epoch_id, num_step=num_step)
